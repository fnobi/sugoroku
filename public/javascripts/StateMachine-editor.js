...
StateMachine.prototype.renderHeader = function () {
        ...
        var $userheader = $('#sugoroku-user', $header);
        $userheader.remove();

        $header.empty();

        var $menu = $([
                '<nav class="sugoroku-menu">',
                '  <ul>',
                '    <li><button id="save-button">save</button></li>',
                '    <li><button id="new-state-button">new state</button></li>',
                '    <li class="select">src: <select id="src-list">',
                tpl(
                        '<option>{{name}}</option>',
                        toNames(this.src)
                ),
...
StateMachine.prototype.save = function (callback) {
        ...
        var pageInfo = fetchPageInfo();
        var codeName = pageInfo.codeName;
        var userName = pageInfo.userName;
        ...
        if (!userName) {
                alert('sorry, fail to fetch user name.');
                return;
        }
...
var fetchPageInfo = function () {
        var location = window.location + '';
        var locationMatch = location.match(/editor\/([\w]+)\/([\w]+)/);
        ...
        return {
                userName: locationMatch[1],
                codeName: locationMatch[2]
        };
};
